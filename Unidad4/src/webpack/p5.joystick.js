...
p5.prototype.createJoystick = function (debug) {
  ...
  function calculateCoordinates(x, y, w, h) {
    if (x == prevX && y == prevY && w == prevW && h == prevH) return;
    ...
    coordinates = {
      ...
      select: { x: x + aW * 0.1, y: y, w: aW, h: aW / 2 },
    };
    loadCalibration();
  }
    ...
    if (savedCalibration) {
      ...
      for (const [gamepadId, joystick] of Object.entries(calibration)) {
        for (const [name, inputButton] of Object.entries(joystick)) {
          if (coordinates[inputButton]) coordinates[inputButton].button = name;
        }
      }
    }
  };
  ...
  saveCalibration = function (name, gamepadId, inputButton) {
    ...
    if (!(gamepadId in calibration)) calibration[gamepadId] = {};
  ...
  _this.getButtonPressedByIndex = function (gamepadIndex, buttonIndex) {
    if (
      controllers[gamepadIndex] &&
      controllers[gamepadIndex].gamepad.buttons[buttonIndex]
    )
  ...
  _this.getAxesValueByIndex = function (gamepadIndex, axesIndex) {
    if (
      controllers[gamepadIndex] &&
      controllers[gamepadIndex].gamepad.axes[axesIndex]
    )
  ...
  _this.draw = function (x, y, w, h) {
    if (!x || !y)
      console.error(
        "p5joystick draw() function: parameters x and y are required"
      );
    ...
    with (coordinates) {
      ...
      circle(
        buttonYellow.x + buttonYellow.w / 2,
        buttonYellow.y + buttonYellow.h / 2,
        buttonYellow.w
      );

      fill(buttonBlue.pressed ? color(127, 127, 255) : color(0, 0, 255));
      circle(
        buttonBlue.x + buttonBlue.w / 2,
        buttonBlue.y + buttonBlue.h / 2,
        buttonBlue.w
      );

      fill(buttonRed.pressed ? color(255, 127, 127) : color(255, 0, 0));
      circle(
        buttonRed.x + buttonRed.w / 2,
        buttonRed.y + buttonRed.h / 2,
        buttonRed.w
      );

      fill(buttonGreen.pressed ? color(127, 255, 127) : color(0, 255, 0));
      circle(
        buttonGreen.x + buttonGreen.w / 2,
        buttonGreen.y + buttonGreen.h / 2,
        buttonGreen.w
      );
    ...
    if (calibrating) {
      for (const [name, button] of Object.entries(coordinates)) {
        ...
        stroke(0);
        if (button.selected) fill(0, 255, 255, 100);
        else fill(0, 100);
        ...
        if (button.button) {
          ...
          text(
            button.button,
            button.x + button.w / 2 - 10,
            button.y + button.h / 2 + 2
          );
        }
      }
    }
    ...
    for (j in controllers) {
      ...
      for (i = 0; i < gamepad.axes.length; i++) {
        ...
        if (axesPrevState[i] != axesValue) {
          ...
          if (eventObj.pressed)
            eventObj.name = getCalibrationName(
              gamepad.id,
              "a" + i + ":" + axesValue
            );
          else
          ...
          } else axesReleasedHandlers.forEach((func) => func(eventObj));
